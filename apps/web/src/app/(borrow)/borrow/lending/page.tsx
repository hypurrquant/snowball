...
import type { UnifiedBorrowMarket } from "@/domains/defi/unified/types";
...
function MarketRow({ market }: { market: UnifiedBorrowMarket }) {
  const href = market.protocol === "morpho" ? "/morpho/borrow" : "/aave/borrow";
    ...
    <Link href={href}>
      <div className="group bg-bg-card/60 backdrop-blur-xl border border-border hover:border-ice-400/30 rounded-xl p-4 flex items-center gap-4 transition-all duration-200 cursor-pointer">
        ...
        <div className="text-right shrink-0">
          <div className="text-xs text-text-secondary mb-0.5">Borrow APY</div>
          <div className="text-warning font-mono font-semibold text-base">
        ...
        <div className="text-right shrink-0 hidden md:block">
          <div className="text-xs text-text-secondary mb-0.5">Borrow APR</div>
          <div className="font-mono text-sm text-white">
            {formatNumber(market.borrowAPR)}%
          </div>
        </div>

        {/* Total Borrow */}
        <div className="text-right shrink-0 hidden sm:block">
          <div className="text-xs text-text-secondary mb-0.5">Total Borrow</div>
          <div className="font-mono text-sm text-white">
            {formatTokenAmount(market.totalBorrow, market.assetDecimals, 2)}{" "}
  ...
  return (
    <div className="bg-bg-card/60 border border-border rounded-xl p-4 flex items-center gap-4 animate-pulse">
      ...
      <div className="text-right space-y-2 shrink-0 hidden md:block">
        <div className="h-3 bg-bg-input rounded w-14" />
        <div className="h-4 bg-bg-input rounded w-12" />
      </div>
...
export default function BorrowLendingPage() {
  const { markets, isLoading } = useUnifiedBorrowMarkets();
    ...
    <div className="max-w-5xl mx-auto px-4 py-8 relative space-y-6">
      ...
      <div>
        <h1 className="text-2xl font-bold flex items-center gap-2 text-white">
          <CreditCard className="w-6 h-6 text-ice-400" />
          Borrow Markets
        </h1>
        <p className="text-text-secondary mt-1 text-sm">
          Find the best borrowing rates
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
        ...
        <Card className="bg-bg-card/60 border-border">
          <CardContent className="p-4">
            <div className="text-xs text-text-secondary mb-1">Lowest APY</div>
            <div className="text-xl font-bold text-warning font-mono">
              {isLoading || markets.length === 0
                ? "—"
                : `${formatNumber(markets[0].borrowAPY)}%`}
            </div>
          </CardContent>
        </Card>
      ...
      <p className="text-xs text-text-secondary">
        Sorted by lowest borrow rate first
      </p>
      ...
      <div className="space-y-2">
        {isLoading ? (
          ...
          <div className="bg-bg-card/60 border border-border rounded-xl p-10 text-center">
            <p className="text-text-secondary">No borrow markets available yet.</p>
          </div>
        ) : (
