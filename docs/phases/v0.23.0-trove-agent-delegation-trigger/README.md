...
### 목표
1. Liquity Borrow 페이지의 각 Trove 카드에 "Agent 위임" 버튼을 추가하여 기존 위임 위저드(`/agent/delegate/{agentId}`)로 라우팅한다 (troveId 전달)
2. Trove 카드에 현재 위임 상태를 표시한다 (on-chain 조회: `getAddManagerOf`, `getInterestIndividualDelegateOf`)
3. 이미 위임된 Trove는 위임 상태 뱃지를 보여주고, 해제 옵션을 제공한다

### 비목표 (Out of Scope)
- 새로운 위임 위저드 개발 (기존 DelegationSetupWizard 재사용)
- Agent 선택/검색 UI 신규 개발
- Agent 런타임/서버 변경
