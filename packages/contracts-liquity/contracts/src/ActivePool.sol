    ...
    */
    ...
    function _mintAggInterest(uint256 _upfrontFee) internal returns (uint256 mintedAmount) {
        ...
        if (mintedAmount > 0) {
            ...
            sbUSDToken.mint(address(interestRouter), remainderToLPs);

            if (spYield > 0) {
                sbUSDToken.mint(address(stabilityPool), spYield);
        ...
        internal
    {
        ...
        if (_troveChange.batchAccruedManagementFee > 0) {
            sbUSDToken.mint(_batchAddress, _troveChange.batchAccruedManagementFee);
        }
