...
contract BorrowerOperations is LiquityBase, AddRemoveManagers, IBorrowerOperations {
    ...
    */
    ...
    event SbUSDTokenAddressChanged(address _sbUSDTokenAddress);
    ...
    function _openTrove(
        ...
        _change.debtIncrease = _sbUSDAmount;
        ...
        // Mint the requested _sbUSDAmount to the borrower and mint the gas comp to the GasPool
        vars.sbUSDToken.mint(msg.sender, _sbUSDAmount);
    ...
    function withdrawBold(uint256 _troveId, uint256 _sbUSDAmount, uint256 _maxUpfrontFee) external override {
        ITroveManager troveManagerCached = troveManager;
        _requireTroveIsActive(troveManagerCached, _troveId);

        TroveChange memory troveChange;
        troveChange.debtIncrease = _sbUSDAmount;
    ...
    function repayBold(uint256 _troveId, uint256 _sbUSDAmount) external override {
        ITroveManager troveManagerCached = troveManager;
        _requireTroveIsActive(troveManagerCached, _troveId);

        TroveChange memory troveChange;
        troveChange.debtDecrease = _sbUSDAmount;
    ...
    */
    function _adjustTrove(
        ...
        if (_troveChange.debtDecrease > 0) {
            ...
            _requireSufficientBoldBalance(vars.sbUSDToken, msg.sender, _troveChange.debtDecrease);
        }
        ...
        _moveTokensFromAdjustment(receiver, _troveChange, vars.sbUSDToken, vars.activePool);
    }

    function closeTrove(uint256 _troveId) external override {
        ITroveManager troveManagerCached = troveManager;
        IActivePool activePoolCached = activePool;
        ISbUSDToken sbUSDTokenCached = sbUSDToken;
        ...
        _requireSufficientBoldBalance(sbUSDTokenCached, msg.sender, trove.entireDebt);
        ...
        sbUSDTokenCached.burn(msg.sender, trove.entireDebt);
    ...
    function _moveTokensFromAdjustment(
        ...
        TroveChange memory _troveChange,
        ISbUSDToken _sbUSDToken,
        IActivePool _activePool
    ) internal {
        if (_troveChange.debtIncrease > 0) {
            _sbUSDToken.mint(withdrawalReceiver, _troveChange.debtIncrease);
        } else if (_troveChange.debtDecrease > 0) {
            _sbUSDToken.burn(msg.sender, _troveChange.debtDecrease);
        }
    ...
    function _requireSufficientBoldBalance(ISbUSDToken _sbUSDToken, address _borrower, uint256 _debtRepayment)
        internal
        view
    {
        if (_sbUSDToken.balanceOf(_borrower) < _debtRepayment) {
