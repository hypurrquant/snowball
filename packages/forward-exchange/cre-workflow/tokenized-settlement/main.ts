...
const onCronTrigger = (runtime: Runtime<Config>, payload: CronPayload): object => {
	...
	// Step 1: Get active (unsettled) series IDs
	runtime.log('Fetching active series from MaturityTokenFactory...')
	...
	if (resultHex === '0x' || resultHex.length < 4) {
		runtime.log('Factory contract not deployed or returned empty data. Skipping.')
		return { settled: 0, status: 'contract_not_found' }
	}
	...
	runtime.log(`Found ${activeSeriesIds.length} active series`)

	if (activeSeriesIds.length === 0) {
		runtime.log('No active series to settle')
		return { settled: 0 }
	}
	...
	// Convert rates to 18 decimal format
