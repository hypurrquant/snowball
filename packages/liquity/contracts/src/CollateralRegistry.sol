...
contract CollateralRegistry is ICollateralRegistry {
    ...
    ISbUSDToken public immutable sbUSDToken;
    ...
    constructor(ISbUSDToken _sbUSDToken, IERC20Metadata[] memory _tokens, ITroveManager[] memory _troveManagers) {
    ...
    function redeemCollateral(uint256 _sbUSDAmount, uint256 _maxIterationsPerCollateral, uint256 _maxFeePercentage)
        external
    {
        ...
        uint256 redemptionRate =
            _calcRedemptionRate(_getUpdatedBaseRateFromRedemption(_sbUSDAmount, totals.boldSupplyAtStart));
        ...
        for (uint256 index = 0; index < totals.numCollaterals; index++) {
            ...
            if (unbackedPortions[index] > 0) {
                uint256 redeemAmount = _sbUSDAmount * unbackedPortions[index] / totals.unbacked;
                if (redeemAmount > 0) {
                    ITroveManager troveManager = getTroveManager(index);
                ...
                // Ensure that per-branch redeems add up to `_sbUSDAmount` exactly
