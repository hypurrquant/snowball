...
contract MetadataNFT is IMetadataNFT {
    ...
    function uri(TroveData memory _troveData) public view returns (string memory) {
        ...
        return json.formattedMetadata(
            string.concat("Snowball - ", IERC20Metadata(_troveData._collToken).name()),
            string.concat(
                "Snowball is a collateralized debt platform. Users can lock up ",
                IERC20Metadata(_troveData._collToken).symbol(),
                " to issue stablecoin tokens (sbUSD) to their own address. The individual collateralized debt positions are called Troves, and are represented as NFTs."
            ),
    ...
    function attributes(TroveData memory _troveData) public pure returns (string memory) {
        ...
        return string.concat(
            ...
            LibString.toHexString(_troveData._sbUSDToken),
