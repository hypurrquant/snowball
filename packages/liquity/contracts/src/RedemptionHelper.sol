...
import {ISbUSDToken} from "./Interfaces/ISbUSDToken.sol";
...
contract RedemptionHelper is IRedemptionHelper {
    ...
    constructor(ICollateralRegistry _collateralRegistry, IAddressesRegistry[] memory _addresses) {
        ...
        sbUSDToken = _collateralRegistry.sbUSDToken();
    ...
    function redeemCollateral(
        ...
        uint256 boldBalanceBefore = sbUSDToken.balanceOf(address(this));

        sbUSDToken.transferFrom(msg.sender, address(this), _bold);
        ...
        uint256 boldRemaining = sbUSDToken.balanceOf(address(this)) - boldBalanceBefore;
        if (boldRemaining > 0) sbUSDToken.transfer(msg.sender, boldRemaining);
    }
}
