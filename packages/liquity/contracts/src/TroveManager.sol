...
contract TroveManager is LiquityBase, ITroveManager, ITroveEvents {
    ...
    ISbUSDToken internal sbUSDToken;
    ...
    /*
    * L_coll and L_sbUSDDebt track the sums of accumulated liquidation rewards per unit staked. During its lifetime, each stake earns:
    *
    * An Coll gain of ( stake * [L_coll - L_coll(0)] )
    * A sbUSDDebt increase  of ( stake * [L_sbUSDDebt - L_sbUSDDebt(0)] )
    *
    * Where L_coll(0) and L_sbUSDDebt(0) are snapshots of L_coll and L_sbUSDDebt for the active Trove taken at the instant the stake was made
    */
    uint256 internal L_coll;
    uint256 internal L_sbUSDDebt;
    ...
    **/
    ...
    event SbUSDTokenAddressChanged(address _newSbUSDTokenAddress);
    ...
    constructor(IAddressesRegistry _addressesRegistry) LiquityBase(_addressesRegistry) {
        ...
        sbUSDToken = _addressesRegistry.sbUSDToken();
        ...
        emit SbUSDTokenAddressChanged(address(sbUSDToken));
    ...
    function _applySingleRedemption(
        ...
        if (_isTroveInBatch) {
            emit BatchedTroveUpdated({
                ...
                _snapshotOfTotalDebtRedist: L_sbUSDDebt
            });
        } else {
            emit TroveUpdated({
                _troveId: _singleRedemption.troveId,
    ...
    */
    ...
    function urgentRedemption(uint256 _sbUSDAmount, uint256[] calldata _troveIds, uint256 _minCollateral) external {
        _requireIsShutDown();
        _requireAmountGreaterThanZero(_sbUSDAmount);
        _requireBoldBalanceCoversRedemption(sbUSDToken, msg.sender, _sbUSDAmount);
        ...
        uint256 remainingBold = _sbUSDAmount;
        ...
        emit Redemption(_sbUSDAmount, totalsTroveChange.debtDecrease, totalsTroveChange.collDecrease, 0, price, price);
        ...
        // Burn bold
        sbUSDToken.burn(msg.sender, totalsTroveChange.debtDecrease);
    }
    ...
    function _updateTroveRewardSnapshots(uint256 _troveId) internal {
        ...
        rewardSnapshots[_troveId].sbUSDDebt = L_sbUSDDebt;
    }
    ...
    function _getLatestTroveData(uint256 _troveId, LatestTroveData memory trove) internal view {
        ...
        trove.redistBoldDebtGain = stake * (L_sbUSDDebt - rewardSnapshots[_troveId].sbUSDDebt) / DECIMAL_PRECISION;
    ...
    function _getLatestTroveDataFromBatch(
        ...
        _latestTroveData.redistBoldDebtGain =
            stake * (L_sbUSDDebt - rewardSnapshots[_troveId].sbUSDDebt) / DECIMAL_PRECISION;
    ...
    function _redistributeDebtAndColl(
        ...
        * error correction, to keep the cumulative error low in the running totals L_coll and L_sbUSDDebt:
        *
        ...
        */
        ...
        uint256 sbUSDDebtNumerator = _debtToRedistribute * DECIMAL_PRECISION + lastBoldDebtError_Redistribution;

        // Get the per-unit-staked terms
        uint256 collRewardPerUnitStaked = collNumerator / totalStakes;
        uint256 sbUSDDebtRewardPerUnitStaked = sbUSDDebtNumerator / totalStakes;

        lastCollError_Redistribution = collNumerator - collRewardPerUnitStaked * totalStakes;
        lastBoldDebtError_Redistribution = sbUSDDebtNumerator - sbUSDDebtRewardPerUnitStaked * totalStakes;

        // Add per-unit-staked terms to the running totals
        L_coll = L_coll + collRewardPerUnitStaked;
        L_sbUSDDebt = L_sbUSDDebt + sbUSDDebtRewardPerUnitStaked;
    ...
    */
    ...
    function _requireBoldBalanceCoversRedemption(ISbUSDToken _sbUSDToken, address _redeemer, uint256 _amount)
        internal
        view
    {
        uint256 boldBalance = _sbUSDToken.balanceOf(_redeemer);
