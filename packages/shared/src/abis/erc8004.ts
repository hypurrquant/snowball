...
export const AgentVaultABI = [
  ...
  "function grantPermission(address agent, address[] targets, bytes4[] functions, uint256 expiry, tuple(address token, uint256 cap)[] tokenCaps)",
  "function revokePermission(address agent)",
  "function setTokenAllowances(address agent, tuple(address token, uint256 cap)[] tokenCaps)",
  "function executeOnBehalf(address user, address target, bytes data) returns (bytes)",
  "function approveAndExecute(address user, address token, uint256 amount, address target, bytes data) returns (bytes)",
  "function transferFromVault(address user, address token, address to, uint256 amount)",
  "function getPermission(address user, address agent, address[] tokens) view returns (tuple(address[] allowedTargets, bytes4[] allowedFunctions, uint256 expiry, bool active, tuple(address token, uint256 cap, uint256 spent)[] tokenAllowances))",
  "function getTokenAllowance(address user, address agent, address token) view returns (uint256 cap, uint256 spent)",
  "function getPermNonce(address user, address agent) view returns (uint256)",
  "function getBalance(address user, address token) view returns (uint256)",
  "function getDelegatedUsers(address agent) view returns (address[])",
  "event Deposited(address indexed user, address indexed token, uint256 amount)",
  "event Withdrawn(address indexed user, address indexed token, uint256 amount)",
  "event PermissionGranted(address indexed user, address indexed agent, address[] targets, bytes4[] functions, uint256 expiry, tuple(address token, uint256 cap)[] tokenCaps)",
  "event PermissionRevoked(address indexed user, address indexed agent)",
  "event TokenAllowancesUpdated(address indexed user, address indexed agent, tuple(address token, uint256 cap)[] tokenCaps)",
