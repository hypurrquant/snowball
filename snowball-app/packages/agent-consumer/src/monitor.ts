...
import { executeForUser, isUserRegistered, getServerWallet } from "./privyHelper";
...
export class PositionMonitor {
    ...
    try {
      ...
      try {
        if (isUserRegistered(address)) {
          ...
          const result = await executeForUser(address, boAddress, calldata);
          monitorLogger.info(
  ...
  private async poll() {
    ...
    for (const [address, { strategy, agentId }] of this.positions) {
      try {
        // Query using the server wallet address (trove owner) when available
        const sw = getServerWallet(address);
        const queryAddress = sw?.serverWalletAddress ?? address;
        const positions = await getUserPositions(queryAddress);
